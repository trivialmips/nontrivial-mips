...
_start:
    ...
    swl $3, 0($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x1234560d

    sw  $2, 0($s0)       # ans: [0x1000]=0x12345678
    swl $3, 1($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x12340d1c

    sw  $2, 4($s0)       # ans: [0x1004]=0x12345678
    swl $3, 6($s0)       # ans: skip
    lw  $4, 4($s0)       # ans: $4=0x120d1c2b

    sw  $2, 4($s0)       # ans: [0x1004]=0x12345678
    swl $3, 7($s0)       # ans: skip
    lw  $4, 4($s0)       # ans: $4=0x0d1c2b3a

    sw  $2, 0($s0)       # ans: [0x1000]=0x12345678
    swr $3, 3($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x3a345678

    sw  $2, 0($s0)       # ans: [0x1000]=0x12345678
    swr $3, 2($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x2b3a5678

    sw  $2, 0($s0)       # ans: [0x1000]=0x12345678
    swr $3, 1($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x1c2b3a78

    sw  $2, 0($s0)       # ans: [0x1000]=0x12345678
    swr $3, 0($s0)       # ans: skip
    lw  $4, 0($s0)       # ans: $4=0x0d1c2b3a
